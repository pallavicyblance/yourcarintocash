...
class Acceptedaps:
    ...
    def saveConditionReportAdd(self,data):
        ...
        cursor1.execute("SELECT zipcode FROM condition_zipcode where sessionid = %s ", (data['sessionid']))
        zipalllist = cursor1.fetchall()
        if len(zipalllist) != 0:
            arr = np.array(zipalllist)               
            new_final_zip = (",".join(arr[:, 0]))
            my_list_new_arr = new_final_zip.split(",")
            #print(my_list_new_arr)  
            unique_list = list(set(my_list_new_arr)) 
            allZip = (",".join(set(unique_list))) 
