...
class Setting:
    ...
    def update(self, id, data):
        ...
        try:
            cursor.execute(
                "UPDATE setting set p1 = %s, p2 = %s, p3 = %s , p4 = %s , p5 = %s,  p6 = %s ,  p1_min = %s,  p1_max = %s,  p2_min = %s,  p2_max = %s,  p3_min = %s,  p3_max = %s,  p4_min = %s,  p4_max = %s,  p5_min = %s,  p5_max = %s,  p6_max = %s where id = %s",
                (data['p1'], data['p2'], data['p3'], data['p4'], data['p5'], data['p6'], data['p1_min'], data['p1_max'],
                 data['p2_min'], data['p2_max'], data['p3_min'], data['p3_max'], data['p4_min'], data['p4_max'],
                 data['p5_min'], data['p5_max'], data['p6_max'], id,))
            con.commit()
            return True
        except:
            con.rollback()
    ...
    def getmakes(self, data):
        ...
        try:
            cursor.execute(
                "SELECT m.id makeId, m.name make, m.shortcode makecode FROM makes m LEFT Join vehiclespecschema_year vssy ON m.id = vssy.make_id Where vssy.year = %s",
                (data['year'],))
            return cursor.fetchall()
        except:
            return ()
    ...
    def getmodels(self, data):
        ...
        try:
            cursor.execute(
                "SELECT m.id makeId, m.name make, md.name model, md.id modelId FROM makes m LEFT Join vehiclespecschema_year vssy ON m.id = vssy.make_id LEFT JOIN vehiclespecschema_model vssm ON vssm.vssy_id = vssy.id LEFT JOIN models md ON md.id = vssm.modelId  Where vssy.year = %s AND m.id = %s ",
                (data['year'], data['make'],))
            return cursor.fetchall()
        except:
            return ()
    ...
    def getKip(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT count(DATE_FORMAT(`created_at`, '%Y-%m-%d')) FROM `accepted_aps` WHERE DATE_FORMAT(`created_at`, '%Y-%m-%d') = CURDATE()")
            return cursor.fetchall()
    ...
    def getKipMonth(self):
        ...
        try:
            #return current_time1;
            cursor.execute(
                "select COUNT(*) from `accepted_aps` where MONTH(`created_at`) = MONTH(now()) and YEAR(`created_at`) = YEAR(now())")
            return cursor.fetchall()
    ...
    def getTranslateTxt(self, lang):
        ...
        if lang == 'es':
            inAuctionstatus = [{'Home': 'Hogar', 'How_it_Works': 'Cómo funciona', 'Faqs': 'Preguntas Frecuentes',
                                'About_Us': 'Acerca de Nosotros', 'Contact_Us': 'Contáctenos',
                                'header_btn_txt': 'Obtén una oferta instantánea por tu coche ahora'
                                   , 'sidebar_heading': 'Obtén una Oferta Instantánea',
                                'sidebar_description': "Llevaremos un registro de tus respuestas aquí. Puedes volver a una pregunta anterior en cualquier momento."
            ...
            inAuctionstatus = [{'Home': 'Home', 'How_it_Works': 'How it Works', 'Faqs': 'Faqs', 'About_Us': 'About Us',
                                'Contact_Us': 'Contact Us', 'header_btn_txt': 'Get an Instant Offer on you car now'
                                   , 'sidebar_heading': 'Get an Instant Offer',
                                'sidebar_description': "We'll keep track of your answers over here. You can jump back to a previous question any time."
    ...
    def getCount(self, startDate, endDate):
        try:
            ...
            cursor.execute(
                'SELECT COUNT(id) from accepted_aps where status = "accept" AND created_at BETWEEN %s AND %s',
                (startDate, endDate))
            accepted_count = cursor.fetchone()[0]

            cursor.execute(
                'SELECT COUNT(id) from accepted_aps where status_update = "Pending pick up" AND created_at BETWEEN %s AND %s',
                (startDate, endDate))
            pending_count = cursor.fetchone()[0]

            cursor.execute(
                'SELECT COUNT(id) from accepted_aps where revised_price IS NOT NULL AND created_at BETWEEN %s AND %s',
                (startDate, endDate))
            offer_given_count = cursor.fetchone()[0]

            cursor.execute(
                'SELECT COUNT(id) from accepted_aps where status_update = "At auction" AND created_at BETWEEN %s AND %s',
                (startDate, endDate))
            at_auction_count = cursor.fetchone()[0]

            cursor.execute(
                'SELECT COUNT(id) from accepted_aps where status_update = "Canceled" AND created_at BETWEEN %s AND %s',
                (startDate, endDate))
    ...
    def getInquiryData(self, start_date, end_date, status, start, length, column, direction, search_value):
        ...
        try:
            ...
            start_date, end_date, '%' + search_value + '%', '%' + search_value + '%', '%' + search_value + '%',
            '%' + search_value + '%', '%' + search_value + '%', '%' + search_value + '%', int(length), int(start)))
    ...
    def get_total_records(self, start_date, end_date, status, search_value):
        ...
        try:
            ...
            query_params = [start_date, end_date, search_value, search_value, search_value, search_value, search_value,
                            search_value]
            ...
            if status == 'accept':
                ...
                '''.format(concat_columns)
                query_params = [start_date, end_date, search_value, search_value, search_value, search_value,
                ...
                '''.format(concat_columns)
                query_params = [status_text[status], start_date, end_date, search_value, search_value, search_value,
