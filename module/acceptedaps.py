...
class Acceptedaps:
        ...
        try:
            if id == None:  
                if param:
                    if status=='monthly':
                        query = """
                            SELECT id, year, model, make, zip, original_price, status, user_city, user_state, created_at, revised_price, offer_id, dispatched, ref_id, status_update, utm_source, utm_medium, utm_campaign, currunt_url	
                            FROM accepted_aps
    ...
    def autoinquirysave(self, data):
        ...
        try:            
            if data['record_id'] == '':
                ...
                cursor.execute("INSERT INTO accepted_aps(year, makeid, modelid, make, model, make_code, vin, ipaddr, hostname,created_at,user_city,user_state,user_country,ref_id,utm_source,utm_medium, utm_campaign,currunt_url) VALUES(%s, %s, %s, %s, %s, %s, %s, %s,  %s,  %s, %s,%s,%s,%s, %s, %s, %s, %s)", (data['year'], data['make_id'], data['model_id'], data['make'], data['model'],  data['make_code'], data['vin'],  data['ipaddr'], data['hostname'],datetime.now(),data['user_city'],data['user_state'],data['user_country'],aaa,data['utm_source'],data['utm_medium'],data['utm_campaign'],data['currunt_url']))
                con.commit()
    ...
    def saveConditionReportAdd(self,data):
        ...
        if 'ACV' in data.getlist('condition_type[]') and 'Copart' in data.getlist('condition_type[]'):
            condition_type = 'both'
        else:
            condition_type = data.getlist('condition_type[]')
        ...
        if data['condition_report_id']=='':        
            cursor.execute("INSERT INTO condition_report(title, condition_type, estimate, not_to_exceed_type_action, not_to_exceed_type, not_to_exceed_per, not_to_exceed, fixed_amt_txt, make_model, min_year, max_year, min_mileage, max_mileage, zip, damage, damageimg, make_name, model_name, make1, model1, sdamageImg_s, range_zip, damageComma, airbagComma, driveComma, getSDamageComma, keyComma, titleComma, firDamageComma, unable_to_verify, state, country, final_zip, sessionid ) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",(data['filter_title'],condition_type,estimate,data['not_to_exceed_type_action'], data['not_to_exceed_type'], data['not_to_exceed_per'], data['not_to_exceed'] , data['fixed_amt_txt'], abc, data['year_min'],data['year_max'],data['mileage_min'],data['mileage_max'],data['zip'],getDamage1,abc1,make_name1,model_name1,make_id_s,model_id_s,sdamageImg_s,rangearray,damageComma,airbagComma,driveComma,getSDamageComma,keyComma,titleComma,firDamageComma,unable_to_verify_data, stateComma,usa_data, allZip, sessionid ))           
            con.commit()
    ...
    def remove_zipcode(self, data):
        """
        This function removes a zipcode from the condition_zipcode table based on the sessionid and id provided in the data parameter.
        
        Parameters:
        - data: A dictionary containing the sessionid and id of the zipcode to be removed.
        
        Returns:
        - None if the deletion is successful.
        - An empty tuple () if an exception occurs during the deletion.
        """
        con = Acceptedaps.connect(self)
        cursor = con.cursor()
        try:
            cursor.execute('DELETE FROM condition_zipcode WHERE sessionid = %s AND 	numbers = %s', (data['sessionid'],data['id']))
            con.commit()
        except:
            return ()
        finally:
