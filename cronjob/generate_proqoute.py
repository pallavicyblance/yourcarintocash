        ...
        try:
            ...
            if auction[38] is None:
                ...
                if float(proQuote) <= data_of_setting[3]:
                    p1 = float(data_of_setting[1])
                    proQuote = float(proQuote)
                    proqoute = (proQuote * p1) / 100
                    minPrice = '0'
                    maxPrice = data_of_setting[3]
                    perPrice = p1

                elif float(proQuote) >= data_of_setting[5] and float(proQuote) <= data_of_setting[6]:
                    p2 = float(data_of_setting[4])
                    proQuote = float(proQuote)
                    proqoute = (proQuote * p2) / 100
                    ...
                    p3 = float(data_of_setting[7])
                    proQuote = float(proQuote)
                    proqoute = (proQuote * p3) / 100
                    ...
                    p4 = float(data_of_setting[16])
                    proQuote = float(proQuote)
                    proqoute = (proQuote * p4) / 100
                ...
                for k1 in condition1:
                    ...
                    if k1[19] == "Fixed Amount":
                        ...
                        if k1[20] == 'minus':
                            ...
                            if k1[21] == 'percentage':
                                con_per = '%'
                                per1 = float((proQuote * float(k1[22])) / 100)
                                total1 = float(proQuote) - per1
                                if float(k1[23]) > total1:
                                    proqoute = total1
                                else:
                ...
                cursor.execute('UPDATE  auctions SET pro_qoute_amount = %s WHERE auction_id = %s',
                               (con_amt, auction[4]))
                con.commit()
                print(auction[4], ' -> auction proqoute', con_amt)
            else:
                print(auction[4], 'auction proqoute already generated')

        except Exception as e:
            print('PROQUOTE ERROR:')
            print(e)
            return ()
