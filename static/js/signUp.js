						...
						$.ajax({
							...
							success: function (responses) {
								...
								if(response.status){
									...
									console.log('in')
									var cityname = capitalizeFLetter(response['data'][8]);
									var statename = capitalizeFLetter(response['data'][9]);
									$('#pdamagediv').show();
									$('#d_title').show();
				          			$('#d_title1').hide();
									$("#v_zip_code").val(response['data'][5])
									$("#v_zip1").val(response['data'][5])
									$("#cityinput").val(cityname)
									$('#states_drop').val(response['data'][9]).trigger('change');
...
function nextsteps(id,s=null) {
	...
	if(id == 11 ){
        ...
        alert(paramValue)
        alert(model_selected)
        
        if ((paramValue !== null && paramValue !== '' && paramValue != undefined & paramValue != 'undefined' & paramValue != '0' & paramValue != 0) || (model_selected!='' && model_selected!=null) || (paramValue!='0') || (paramValue!=0)) {
...
function user_send_mail(){
	$.ajax({
		url: 'http://localhost/ACV-API/usermail.php',
		// url: url4,
		data: $('#formproquotes').serialize(),
		type: 'POST', 
		success: function (response) {
