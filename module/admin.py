...
class Admin:
    ...
    def update(self, id, data, email_noti_data):
        ...
        try:
            cursor.execute(
                "UPDATE admin set first_name = %s, last_name = %s, phone = %s , address = %s , email = %s , email_noti = %s where id = %s",
                (data['first_name'], data['last_name'], data['phone'], data['address'], data['email'], email_noti_data,
                 id,))
            con.commit()
            return True
        except:
    ...
    def valid(self, data):
        ...
        try:
            if data['id']:
                ...
                cursor.execute("SELECT username,email FROM admin WHERE email =%s OR username =%s",
    ...
    def insert(self, data, email_noti_data):
        ...
        try:
            if data['id']:
                # return data
                cursor.execute(
                    "UPDATE admin set first_name = %s, last_name = %s, phone = %s , address = %s , email = %s , role=%s , email_noti=%s where id = %s",
                    (data['first_name'], data['last_name'], data['phone'], data['address'], data['email'], data['role'],
                     email_noti_data, data['id'],))
                con.commit()
                if data['role'] == "Staff":
                ...
                #     return "inserted suuccesss"
                    ...
                    cursor.execute(
                        "INSERT INTO admin(first_name,last_name,phone,address,username,email,password,role,email_noti) VALUES(%s, %s, %s,%s, %s, %s,%s,%s,%s)",
                        (data['first_name'], data['last_name'], data['phone'], data['address'], data['username'],
                         data['email'], data['password'], data['role'], email_noti_data))
                    con.commit()
                    if data['role'] == "Staff":
    ...
    def storeToken(self, jwtToken, id, pubnum_auth_key, pubnum_expiration, pubnum_subscribe_key):
        ...
        try:
            ...
            if fetchone is not None:
                cursor.execute(
                    "UPDATE acv_jwt_token set jwt_token = %s, pubnum_auth_key = %s, pubnum_expiration = %s, pubnum_subscribe_key = %s where user_id = %s",
                    (jwtToken, pubnum_auth_key, pubnum_expiration, pubnum_subscribe_key, id))
            else:
                cursor.execute(
                    "INSERT INTO acv_jwt_token (jwt_token,user_id,pubnum_auth_key,pubnum_expiration,pubnum_subscribe_key) VALUES (%s, %s, %s, %s, %s)",
                    (jwtToken, id, pubnum_auth_key, pubnum_expiration, pubnum_subscribe_key))
            con.commit()
    ...
    def getjwttoken(self, id):
        ...
        try:
            cursor.execute(
                "SELECT jwt_token,user_id,pubnum_auth_key,pubnum_subscribe_key FROM acv_jwt_token where user_id = %s",
                (id))
            return cursor.fetchone()
        except:
            return "error"
