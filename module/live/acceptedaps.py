...
class Acceptedaps:
        ...
        try:
            ...
            if getRecord:              
                ...
                if len(getRecord1) != 0:
                    ...
                    allZip = (",".join(arr[:, 0]))  
                    cursor2.execute("SELECT * FROM condition_zipcode where sessionid = %s ", (data['sessionid']))
                    ...
                    if fetch_all:
                        ...
                        cursor3.execute("INSERT INTO `condition_zipcode` (`sessionid`, `zipcode`) VALUES (%s,%s)",(data['sessionid'], allZip ))
                        con3.commit()
    ...
    def saveConditionReportAdd(self,data):
        ...
        cursor1.execute("SELECT * FROM condition_zipcode where sessionid = %s ", (data['sessionid']))
        zipalllist = cursor1.fetchone()
        if zipalllist:
            allZip = zipalllist[2]        
        if data['condition_report_id']=='':
            print('hihiih')           
            cursor.execute("INSERT INTO condition_report(title,estimate,not_to_exceed_type_action,not_to_exceed_type,not_to_exceed_per,not_to_exceed,fixed_amt_txt,make_model,min_year,max_year,min_mileage,max_mileage,zip,damage,damageimg,make_name,model_name,make1,model1,sdamageImg_s,range_zip,damageComma,airbagComma,driveComma,getSDamageComma,keyComma,titleComma,firDamageComma,unable_to_verify,state,country,final_zip) VALUES( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s , %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s )",(data['filter_title'],estimate,data['not_to_exceed_type_action'], data['not_to_exceed_type'], data['not_to_exceed_per'], data['not_to_exceed'] , data['fixed_amt_txt'], abc, data['year_min'],data['year_max'],data['mileage_min'],data['mileage_max'],data['zip'],getDamage1,abc1,make_name1,model_name1,make_id_s,model_id_s,sdamageImg_s,rangearray,damageComma,airbagComma,driveComma,getSDamageComma,keyComma,titleComma,firDamageComma,unable_to_verify_data, stateComma,usa_data, allZip ))           
            con.commit()
    ...
    def getConditionalFilter(self,):
        ...
        query = "SELECT {} FROM condition_report  order by id desc".format(", ".join(columns))
        cursor.execute(query)
    ...
    def getzipcode(self , data):
        ...
        cursor = con.cursor()
        try:
            cursor.execute("SELECT * FROM `get_location` WHERE DELIVERY_ZIPCODE = %s " , (data['v_zip']))
            return cursor.fetchone()
        except:
            con.rollback()
            return False
