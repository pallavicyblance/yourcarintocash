...
from cronjob.latest_auctions import auction_save
...
UPLOAD_FOLDER1 = os.path.join(path, 'static/chatgptimg')
if not os.path.isdir(UPLOAD_FOLDER): os.mkdir(UPLOAD_FOLDER)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER

if not os.path.isdir(UPLOAD_FOLDER1): os.mkdir(UPLOAD_FOLDER1)
...
def conditionfiltersoftdeleted():
    ...
    if not session.get('admin_logged_in'):
        ...
        role = admin.role(idssss)
        #new code value added end here country
        return render_template('condition-filter-deleted.html',modelresult=modelresult,data=data,states=states,role=role)
...
@app.route('/file-upload1', methods=['POST'])
def ajax_upload_file1():
    ...
    for file in files:
        if file and allowed_file(file.filename):
            ...
            acceptedaps.updateimg(ids,filename)
            success = True
        else:
            errors[file.filename] = '2'
...
@app.route('/runcronejob', methods=['GET'])
def schedulecronjob():
    schedule.every(30).seconds.do(acv_login)
    schedule.every(40).seconds.do(auction_save.latest_auctions)
    # schedule.every(3).minutes.do(won_auction)
    #schedule.every(50).seconds.do(auction_place_bid.acv_auction_place_bid)
    # schedule.every(30).seconds.do(auction_10_min_left)
    # schedule.every(5).seconds.do(auction_1_min_left)
    # schedule.every(45).hours.do(remove_auction)
